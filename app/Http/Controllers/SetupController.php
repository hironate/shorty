...
class SetupController extends Controller {
    ...
    public static function performSetup(Request $request) {
        ...
        $shorty_registration_setting = $request->input('setting:registration_permission');

        if ($shorty_registration_setting == 'no-verification') {
            $shorty_acct_activation = false;
            $shorty_allow_acct_creation = true;
        }
        else if ($shorty_registration_setting == 'none') {
            $shorty_acct_activation = false;
            $shorty_allow_acct_creation = false;
        }
        else if ($shorty_registration_setting == 'email') {
            $shorty_acct_activation = true;
            $shorty_allow_acct_creation = true;
        }
        else {
            return view('error', [
        ...
        $shorty_acct_creation_recaptcha = $request->input('setting:acct_registration_recaptcha');
        $shorty_recaptcha_site_key = $request->input('setting:recaptcha_site_key');
        $shorty_recaptcha_secret_key = $request->input('setting:recaptcha_secret_key');
        ...
        $compiled_configuration = view('env', [
            ...
            'SHORTY_GENERATED_AT' => $date_today,
            'SHORTY_SETUP_RAN' => $shorty_setup_ran,
            ...
            'SHORTY_ALLOW_ACCT_CREATION' => $shorty_allow_acct_creation,
            'SHORTY_ACCT_ACTIVATION' => $shorty_acct_activation,
            'SHORTY_ACCT_CREATION_RECAPTCHA' => $shorty_acct_creation_recaptcha,
            ...
            'SHORTY_RECAPTCHA_SITE_KEY' => $shorty_recaptcha_site_key,
            'SHORTY_RECAPTCHA_SECRET' => $shorty_recaptcha_secret_key,
    ...
    public static function finishSetup(Request $request) {
        ...
        return view('setup_thanks')->with('success', 'Set up completed! Thanks for using shorty!');
    }
}
