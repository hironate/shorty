...
class UserController extends Controller {
    ...
    public function performSignup(Request $request) {
        if (env('SHORTY_ALLOW_ACCT_CREATION') == false) {
            return redirect(route('index'))->with('error', 'Sorry, but registration is disabled.');
        }

        if (env('SHORTY_ACCT_CREATION_RECAPTCHA')) {
            // Verify reCAPTCHA if setting is enabled
            $gRecaptchaResponse = $request->input('g-recaptcha-response');

            $recaptcha = new \ReCaptcha\ReCaptcha(env('SHORTY_RECAPTCHA_SECRET_KEY'));
        ...
        $acct_activation_needed = env('SHORTY_ACCT_ACTIVATION');
